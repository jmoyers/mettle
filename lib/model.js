...
Model.prototype.track = function(keys){
   ...
   for(var i=0; i<keys.length;i++){
      ...
      this.__defineGetter__(keys[i], function(){
         return me.get(keys[i]);
      });
      this.__defineSetter__(keys[i], function(val){
         return me.set(keys[i], val);
      });
   }
}
...
Model.prototype.set = function(){
   ...
   var commit = function(attribs, cb){
      ...
      for (var i=0;i<keys.length;i++) {
         if (~me.attribs.indexOf(keys[i])) {
            me.track([keys[i]]);
         }
      }
      
      if (!quiet) {
         me.emit('change', attribs);
